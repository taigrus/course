...
class FamiliasTableSeederTableSeeder extends Seeder {
	...
	public function run()
	{
		...
		for($i=0; $i<50; $i++)
		{
			...
			$id1=DB::table('familia_programa')->insertGetId(array(
				'folio' 	=> $folio,
			...
			for($j=1; $j<=$aleatorio; $j++)
			{
				...
				$id2=DB::table('integrante_familia')->insertGetId(array(
					'familia_id'	=>$id1,
					'integrante_id'	=>$j,
				...
				if($edad<6)
				{
					$aleatorio=$faker->numberBetween(2,10);
					for($j=1; $j<=$aleatorio; $j++) {
						DB::table('integrante_seguimiento')->insert(array(
							'familia_id' => $id1,
